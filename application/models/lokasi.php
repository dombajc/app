...
ORDER BY tgl_sk DESC";
	...
	public function getNamaLokasiById($id) {
		$cmd = "SELECT lokasi
		FROM
		lokasi
		WHERE
		id_lokasi=id_induk AND id_lokasi!='05' and SUBSTR(id_lokasi,3)='". substr($id,2) ."'";
	return $this->db->query($cmd)->row()->lokasi;
	}

	public function optionarrUppd()
	{
		$opsi = '';
		foreach($this->arrUppd()->result() as $row)
		{
			$opsi .= '<option value="'. $row->id_lokasi .'"> '. $row->lokasi .' </option>';
		}
		return $opsi;

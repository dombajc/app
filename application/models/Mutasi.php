...
v_mutasi where id_mutasi='". $postId ."'";
	...
	public function getPegawaiAktifByLokasi($postLokasi)
	{
		...
		$cmd = "SELECT
			...
			WHERE
			v_mutasi.`aktif`=1 AND t_pegawai.`aktif`=1 AND v_mutasi.id_sts_pegawai='33' AND v_mutasi.`id_jabatan` IN ". $in ." and v_mutasi.id_tujuan_mutasi='". $postLokasi ."' order by t_pegawai.`nama_pegawai`";

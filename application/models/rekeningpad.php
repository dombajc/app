...
GROUP BY kd_rekening ORDER BY kd_rekening";
	...
	// Update Script for e-SAMSAT JATENG



	public function get_list_by( $get_id_tahun, $kd_sync ) {

		$get_tahun = $this->Thanggaran->getDataByID( $get_id_tahun )->row()->th_anggaran;

		$cmd = "SELECT
b.no_rekening, b.nm_rekening, b.kd_inisial
FROM rekening a
LEFT JOIN rekening b ON b.kd_subrekening=a.kd_rekening AND a.th=b.th
WHERE a.kd_inisial LIKE '%". $kd_sync ."%' AND a.th='". $get_tahun ."'
ORDER BY b.no_rekening";
		
		return $this->dbPadOL->query($cmd);
	}

	public function get_induk_by( $get_id_tahun, $kd_sync ) {

		$get_tahun = $this->Thanggaran->getDataByID( $get_id_tahun )->row()->th_anggaran;

		$cmd = "SELECT
no_rekening, nm_rekening
FROM rekening
WHERE kd_inisial LIKE '%". $kd_sync ."%' AND th='". $get_tahun ."'";

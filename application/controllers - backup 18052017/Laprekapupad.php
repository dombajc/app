...
class Laprekapupad extends CI_Controller {
	...
	public function act_report()
	{
		...
		$arrTriwulan = $this->Triwulan->getDataByID($postTriwulan)->row();
		$arrThAnggaran = $this->Thanggaran->getDataByID($postAnggaran)->row();
		
		$batasAwalTriwulan = $arrThAnggaran->th_anggaran .'-'. $arrTriwulan->s_bulan;
		$batasAkhirTriwulan = $arrThAnggaran->th_anggaran .'-'. $arrTriwulan->e_bulan;

		if ( $postLokasi == '05' ) {
			$data['dinamisContent'] = $this->folder.'cetak laporan rekapitulasi per upad - khusus pusat';
			$data['arrPegawai'] = $this->getArrPegawaiPusatByFilter($postLokasi, $postTriwulan, $postAnggaran, $batasAwalTriwulan, $batasAkhirTriwulan);
			...
			$data['arrPegawai'] = $this->getArrPegawaiByFilter($postLokasi, $postTriwulan, $postAnggaran, $batasAwalTriwulan, $batasAkhirTriwulan);
		...
		$data['getTahun'] = $arrThAnggaran->th_anggaran;
		$data['getTriwulan'] = $arrTriwulan->triwulan;
	...
	public function get_pdf()
	{
		...
		$getTahun = $arrThAnggaran->th_anggaran;
		$getTriwulan = $arrTriwulan->triwulan;
		...
		if ( $postLokasi == '05' ) {
			$arrPegawai = $this->getArrPegawaiPusatByFilter($postLokasi, $postTriwulan, $postAnggaran, $batasAwalTriwulan, $batasAkhirTriwulan);
...
<br>';
			...
			$arrPegawai = $this->getArrPegawaiByFilter($postLokasi, $postTriwulan, $postAnggaran, $batasAwalTriwulan, $batasAkhirTriwulan);
...
'. $this->Paraf->getParafLaporan('rd2d', $postLokasi, 2 ) .'
	...
	private function getArrPegawaiByFilter($postLokasi, $postTriwulan, $postAnggaran, $batasAwalTriwulan, $batasAkhirTriwulan)
	{		
		$where = $postLokasi == '99' ? '' : " AND id_lokasi='". $postLokasi ."'";
		$cmd = "SELECT
			nama_pegawai,nip,nama_homebase,id_mutasi,
			t_target_d2d.`total`
			FROM
			v_sort_pegawai_update
			LEFT JOIN t_target_d2d ON
			t_target_d2d.`id_jabatan`=v_sort_pegawai_update.`id_jabatan`
			WHERE t_target_d2d.`id_anggaran`='". $postAnggaran ."' AND t_target_d2d.`id_triwulan`='". $postTriwulan ."' AND IF(
    aktif = 1,
    tgl_sk <= '". $batasAkhirTriwulan ."',
    tgl_selesai_mutasi >= '". $batasAwalTriwulan ."'
  )".
			$where ." order by v_sort_pegawai_update.id_jabatan,id_sts_pegawai";

		return $this->db->query($cmd)->result_array();
	}

	private function getArrPegawaiPusatByFilter($postLokasi, $postTriwulan, $postAnggaran, $batasAwalTriwulan, $batasAkhirTriwulan)
	{
		$cmd = "SELECT
			nama_pegawai,nip,nama_lokasi,id_mutasi,
			t_target_d2d.`total`
			FROM
			v_sort_pegawai_update
			LEFT JOIN t_target_d2d ON
    ...
    tgl_sk <= '". $batasAkhirTriwulan ."',
    tgl_selesai_mutasi >= '". $batasAwalTriwulan ."'
  ) and `id_homebase`='". $postLokasi ."' order by v_sort_pegawai_update.id_jabatan,id_sts_pegawai";

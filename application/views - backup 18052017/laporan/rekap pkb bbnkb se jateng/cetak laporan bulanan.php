...
foreach( $this->Lokasi->arrLokasiIndukDanSamtu() as $lokasi )
{
	...
	echo '<td align="right" width="70px">'. $this->Fungsi->formatangkalaporan($TotalPerRowLokasi) .'</td>
	</tr>';
	$no++;
}
?>
...
foreach( $this->Fungsi->arrKodeJenisPajak() as $kode )
{
	...
	//$TotalPerColumn = 0;
	echo '<td width="70px" align="right">'. $this->Fungsi->formatangkalaporan($TotalPerColumn) .'</td>';
	$arrTotal[$kode['id_jenis']] = $TotalPerColumn;
}
?>
<td align="right" width="70px"><?= $this->Fungsi->formatangkalaporan(array_sum($arrTotal)) ?></td>
</tr>
<tr>
...
foreach( $this->Fungsi->arrKodeJenisPajak() as $kode )
{
	...
	echo '<td width="70px" align="right">'. $this->Fungsi->formatangkalaporan($TotalPerColumnLalu) .'</td>';
	$arrTotal[$kode['id_jenis']] += $TotalPerColumnLalu;
}
?>
<td align="right" width="70px"><?= $this->Fungsi->formatangkalaporan(array_sum($arrTrxLalu)) ?></td>
</tr>
<tr>
...
foreach( $this->Fungsi->arrKodeJenisPajak() as $kode )
{
	...
	echo '<td width="70px" align="right">'. $this->Fungsi->formatangkalaporan($TotalFinalColumn) .'</td>';
}
?>
<td align="right" width="70px"><?= $this->Fungsi->formatangkalaporan(array_sum($arrTotal)) ?></td>
</tr>

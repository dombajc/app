				...
				<div class="form-group">
					<div class="col-sm-12">
						...
						<select id="slctstatus" class="form-control filter input-sm">
							<?= $this->Pd->opsiJenis(" where sub_rek_pd='01'") ?>
						</select>
					</div>
				</div>
				<div class="form-group">
					<div class="col-sm-12">
						PENETAPAN & PENERIMAAN<br>
						<label class="radio-inline"><input type="radio" name="postPenetapan" value="1" checked>Tahun Jalan</label>
						<label class="radio-inline"><input type="radio" name="postPenetapan" value="0">Tahun Lalu (tunggakan)</label>
					</div>
				</div>
				<div class="form-group">
					<div class="col-sm-12">
		...
		<section class="panel">
			...
			<div class="panel-body">
				<form class="form-horizontal">
					...
					<input type="hidden" name="hpenetapan" id="hpenetapan">				
...
<script>
	...
	$(function(){
		...
		$('#btn-tampilkan').click(function(){
			...
			var penetapan = $('input[name=postPenetapan]:checked').val();
			$.ajax({
	            type: 'POST',
	            url: '<?= base_url("get_inputan_pkb") ?>',
	            dataType: "JSON",
	            data: 'bulan='+ bulan +'&lokasi='+ lokasi +'&ia='+ ia +'&jenis='+ jenis +'&penetapan='+ penetapan,
	            success: function(html) {
	                        ...
	                        onUnblock: function() {
	                            if (html.error == '')
	                            {
									...
									if ( induk != '05' ) {
										if ( induk == lokasi )
										{
											keterangan = html.lokasi['lokasi'] +' ( Realisasi Induk + samkel + drive thru + paten + gerai )';
										} else {
											keterangan = '( Realisasi '+ html.lokasi['lokasi'] +' )';
										}
									}

									getHtml += '<div class="col-md-12">'+
										'<h4>'+ html.arrPkb.detil.nama_rekening + ' <small>'+ keterangan +'</small></h4>'+

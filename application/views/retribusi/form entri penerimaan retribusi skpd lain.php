...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$('#btn-open-form').click(function(){
			...
			$.ajax({
				type: 'POST',
				url: '<?= base_url("view_form_rekening_skpd_lain") ?>',
				dataType: "JSON",
				data: "post_thn="+ id_thn +"&post_bln="+ id_bulan +"&post_lokasi="+ id_lokasi,
										...
										if ( value.node != null )
										{
											$.each(value.node, function(key2, value2){
													...
													if ( value2.anakan == 0 )
													{
														...
														'<input type="text" class="form-control input-sm nominal text-right nextinput" name="postjumlah[]" id="realisasi_'+ value2.id_rekening +'" tabIndex="'+ i +'" value="'+ value2.jumlah +'">'+
														'</div>'+
														'</div>';
														i++;
													} else {
		...
		.on('success.form.fv', function(e) {
			...
			if (konfirm == 1) {
				$.ajax({
					type: 'POST',
					url: '<?= base_url("simpan_entrian_retribusi_skpd_lain") ?>',
					dataType: "JSON",
		...
		$('#btn-hapus').click(function() {
			...
			var id_lokasi = $('#slctlokasi').val();
			var id_bulan = $('#slctbulan').val();
			var konfirm = confirm("HAPUS ENTRI DATA ?");
			if ( konfirm == 1 )
			{
				$.ajax({
		            type: 'POST',
		            url: '<?= base_url("hapus_entri_retribusi_skpd_lain") ?>',
		            dataType: "JSON",

				...
				<form class="forms form-horizontal">
		            ...
		            <div class="box-body">
		            	<table class="table">
		            		<thead>
		            			<tr>
			            			<th rowspan="2" width="30%" class="text-center">JABATAN</th>
		            		...
		            		<tbody>
		            			<?php
				            		foreach( $this->Jabatan->getAllData()->result() as $jbtn ) {
				            			echo '<tr>
				            				<td>
				            				<div class="form-group">
											  	<label class="col-sm-12 control-label">'. $jbtn->jabatan .'</label>
											</div>
											</td>';
										
										foreach ( $this->Triwulan->getAllData()->result() as $inpertri ) {
			            					echo '<td>
			            					<input type="hidden" name="hIdJabatan[]" value="'. $jbtn->id_jabatan .'">
			            					<input type="hidden" name="hIdtriwulan[]" value="'. $inpertri->id_triwulan .'">
			            					<div class="col-sm-12">
												<input type="text" name="txtjml[]" id="txtjml_'. $jbtn->id_jabatan .'_'. $inpertri->id_triwulan. '" class="form-control nominal">
											</div>
											</td>';
			            				}
...
<script>
	...
	function getDetilData()
	{
		...
		$.ajax({
            type: 'POST',
            url: '<?= base_url("detil_data_target") ?>',
            dataType: "JSON",

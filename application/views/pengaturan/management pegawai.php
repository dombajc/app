					...
					<div class="col-md-12">
						...
						<div class="box box-info">
							<div class="box-body">
								...
								<div class="form-group" id="div-nrp" style="display:none;">
								  <label class="col-sm-4 control-label">NRP <span class="text-danger">*</span></label>
								  <div class="col-sm-8">
									<input type="text" name="txtnrp" id="txtnrp" class="form-control input-sm">
								  </div>
								</div>
								<div class="form-group">
								...
								<div id="show-only-add">
									...
									<div class="form-group" id="div-homebase">
								...
								<div class="form-group" id="div-nohp">
...
<script>
	$(function(){
		...
		$('#txtnrp').inputmask('99999999',
			{ "onincomplete": function(){ 
				$("#txtnrp").val("");
				$('form').formValidation('revalidateField', 'txtnrp');
			}
		});
		...
		$('form').formValidation({
			...
			fields: {
					...
					validators: {
						callback: {
                            ...
                            callback: function(value, validator, $field) {
                                ...
                                return (sts !== '33' && sts !== '99' ) ? true : (value !== '');
                            }
                        }
					}
				},
				txtnrp: {
					validators: {
						callback: {
                            message: 'NRP harap di isi !',
                            callback: function(value, validator, $field) {
                                var sts = $('form').find('[name="slctstatus"]:checked').val();
                                return (sts !== '66' && sts !== '99' ) ? true : (value !== '');
                            }
                        }
					}
				},
				...
				slcthomebase: {
					validators: {
						callback: {
                            ...
                            callback: function(value, validator, $field) {
                                var sts = $('form').find('[name="haksi"]').val();
                                var sts2 = $('form').find('[name="slctstatus"]:checked').val();
                                return (sts !== 'add' && sts2 !== '66') ? true : (value !== '');
                            }
                        }
					}
				},
				...
				txtnohp: {
					validators: {
                        callback: {
                            ...
                            callback: function(value, validator, $field) {
                                var sts = $('form').find('[name="slctstatus"]:checked').val();
                                return (sts !== '99' && sts !== '33' ) ? true : (value !== '') ;
                            }
                        }
					}
				}
                        ...
                        onUnblock: function() {
                            if (html.error == '')
                            {
								...
								if ( html.id_sts_pegawai == '33' ) {
									$('#txtnip').val(html.nip);
									$('#txttgllahir').val('');
								} else if ( html.id_sts_pegawai == '66' ) {
									$('#txtnrp').val(html.nip);
									$('#txttgllahir').val('');
								} else {
    ...
    function actGetStatus(value) {
    	...
    	if ( value === '33' ){
			...
			$( '#div-homebase, #div-nohp' ).show( 'fast' );
    	}else if ( value === '99' ){
    		$('#div-nip').hide('fast');
    		$('#div-nrp').hide('fast');
			$('#div-tgl-lahir, #div-nohp').show('fast');
    		var $opsipangkat = $("#slctpangkat");
			$opsipangkat.empty();
			...
			$( '#div-homebase' ).show( 'fast' );
    	} else if ( value == '66' ) {
    		$('#div-nrp').show('fast');
    		$('#div-nip').hide('fast');
    		var $opsipangkat = $("#slctpangkat");
			$opsipangkat.empty();
			$opsipangkat.append('<?= $this->Pangkat->opsiPolisi() ?>');
			var $opsijabatan = $("#slctjabatan");
			$opsijabatan.empty();
			$opsijabatan.append('<?= $this->Jabatan->opsiPolisi() ?>');
			$( '#div-homebase, #div-tgl-lahir, #div-nohp' ).hide( 'fast' );
    	}

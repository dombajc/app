...
foreach( $this->Bulan->getAllData()->result() as $bulan )
{
	...
	foreach( $this->Fungsi->arrKodeJenisPajak() as $kode ){
		...
		echo '<td width="70px" align="right">'. $total .'</td>';
		$arrTotalperColumn[$kode['id_jenis']][] = $total;
		$TotalPerRowBulan += $total;
	}
	echo '<td align="right" width="70px">'. $TotalPerRowBulan .'</td>
	</tr>';
	if( $no > 1 ) {
		$no ++;
		$subtotal2 = 0;
		...
		foreach( $this->Fungsi->arrKodeJenisPajak() as $kode ){
			...
			echo '<td width="70px" align="right"><b>'. $subtotal .'</b></td>';
			$subtotal2 += $subtotal;
		}
		echo '<td align="right" width="70px"><b>'. $subtotal2 .'</b></td>
		</tr>';
	}
	$no ++;
}

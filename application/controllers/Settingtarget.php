...
class Settingtarget extends CI_Controller {
	...
	{
		$data['title'] = 'Pengisian Target Minimal Pengiriman Dok. D2D';
		$data['subtitle'] = '';
		$data['dinamisContent'] = $this->folder.'atur target d2d';
	...
	public function act_crud()
	{
		...
		switch ($_POST['haksi']) {
			...
			case 'add':
				...
				$result = $this->Target->tambah($arrPost, $this->security->xss_clean($_POST['slctth']));
				if ( $result == ''){
			...
			case 'delete':
				$result = $this->Target->delete($this->security->xss_clean($_POST['hID']));
				if ( $result == ''){
	...
	public function json_detil()
	{
		...
		if ( $getData->num_rows() == 1 ){
			...
			foreach ( $this->Target->getDataById($row->id_anggaran)->result() as $row2 ) {
				$resultArr[$row2->id_jabatan .'_'. $row2->id_triwulan]['forID'] = $row2->id_jabatan .'_'. $row2->id_triwulan;
				$resultArr[$row2->id_jabatan .'_'. $row2->id_triwulan]['total'] = $row2->total;
			};

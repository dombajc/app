...
class Aturparaf extends CI_Controller {
	...
	public function json_detil()
	{
		...
		$get_location = $this->security->xss_clean( $_POST['lokasi'] );

		$arrPegawai = $this->Mutasi->getPegawaiAktifByLokasi( $get_location )->result_array();
		$arrParaf = $this->Paraf->getDataSettingParaf( $get_location );
		$arrInput = $this->Paraf->get_form_by_location( $get_location )->result();

		$json['error'] = $error;
		$json['arrPegawai'] = $arrPegawai;
		$json['arrParaf'] = $arrParaf;
		$json['arrInput'] = $arrInput;
			...
			case 'add':
				...
				foreach ( $arrInput as $row ) {
					$arrPost[] = array(
						...
						'id_pegawai' => $row[2],
						'id_paraf' => $row[1],
						'nm_plh' => empty($this->security->xss_clean($_POST['txtnm_plh_'. $row[1]])) ? '' : $this->security->xss_clean($_POST['txtnm_plh_'. $row[1]]),
						'nip_plh' => empty($this->security->xss_clean($_POST['txtnip_plh_'. $row[1]])) ? '' : $this->security->xss_clean($_POST['txtnip_plh_'. $row[1]]),
						'pangkat_plh' => empty($this->security->xss_clean($_POST['txtpangkat_plh_'. $row[1]])) ? '' : $this->security->xss_clean($_POST['txtpangkat_plh_'. $row[1]]),
						'jabatan_plh' => empty($this->security->xss_clean($_POST['txtjbtn_plh_'. $row[1]])) ? '' : $this->security->xss_clean($_POST['txtjbtn_plh_'. $row[1]])
					);
					$i++;
				}

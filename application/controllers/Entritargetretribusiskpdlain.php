...
class Entritargetretribusiskpdlain extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = 'ENTRY TARGET RETRIBUSI SKPD LAIN';
		$data['subtitle'] = '';
		$data['dinamisContent'] = $this->folder.'form entri target retribusi skpd lain';
	...
	public function json_view_rekening_skpd_lain()
	{
		...
		$post_target = $this->security->xss_clean($_POST['post_target']);
		...
		$arrEntrian = $this->Targetretribusiskpdlain->arr_entrian_by_rekening($post_thn, $post_target, $post_lokasi);
		$rowSearchEntrian = $this->Targetretribusiskpdlain->find_entrian($post_thn, $post_target, $post_lokasi);
	...
	public function act()
	{
		...
		$post_thn = $this->security->xss_clean($_POST['post_thn']);
		$post_target = $this->security->xss_clean($_POST['posttarget']);
		...
		$datarekam = array(
			...
			'id_target' => $post_target,
			'id_user' => $get_user,
			'last_update' => $get_last_update
		);
		
		$result = $this->Targetretribusiskpdlain->save($datarekam, $arrItem);
	...
	public function hapus()
	{
		...
		$result = $this->Targetretribusiskpdlain->hapus($post_thn, $post_target, $post_lokasi);
